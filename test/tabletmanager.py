...
from vtproto.tabletmanagerdata_pb2 import LockTablesRequest, UnlockTablesRequest, StopSlaveRequest, MasterPositionRequest
  ...
  primary key (id)
    ...
    def test_start_slave_until_after(self):
        ...
        try:
            ...
            _write_data_to_master()
            current_position = master_tablet_manager.MasterPosition(MasterPositionRequest())
            logging.debug('apa %s"', current_position.GetPosition())
        finally:
            replica.kill_vttablet()
            master.kill_vttablet()
