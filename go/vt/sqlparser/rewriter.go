...
func DeepCopySelectStatement(in SelectStatement) SelectStatement {
	switch node := in.(type) {
	case *ParenSelect:
		return &ParenSelect{Select: DeepCopySelectStatement(node.Select)}
	case *Union:
		return &Union{
			Type:    node.Type,
			Left:    DeepCopySelectStatement(node.Left),
			Right:   DeepCopySelectStatement(node.Right),
			OrderBy: DeepCopyOrderBy(node.OrderBy),
			Limit:   DeepCopyLimit(node.Limit),
			Lock:    node.Lock,
		}
	}
