...
func commandBackfillLookup(wi *Instance, wr *wrangler.Wrangler, subFlags *flag.FlagSet, args []string) (Worker, error) {
	...
	worker, err := newBackfillLookupWorker(wr, wi.cell, keyspace, vindexName, topodatapb.TabletType(tabletType), *chunkCount, *minRowsPerChunk, *sourceReaderCount, *writeQueryMaxRows, *writeQueryMaxSize, *destinationWriterCount, *maxTPS, *maxReplicationLag)
	if err != nil {
...
func interactiveBackfillLookup(ctx context.Context, wi *Instance, wr *wrangler.Wrangler, w http.ResponseWriter, r *http.Request) (Worker, *template.Template, map[string]interface{}, error) {
	...
	wrk, err := newBackfillLookupWorker(wr, wi.cell, keyspace, vindex, topodatapb.TabletType(tabletType), int(chunkCount), int(minRowsPerChunk), int(sourceReaderCount), int(writeQueryMaxRows), int(writeQueryMaxSize), int(destinationWriterCount), maxTPS, maxReplicationLag)
	if err != nil {
