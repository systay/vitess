...
func (stats *LogStats) Logf(w io.Writer, params url.Values) error {
	...
	case streamlog.QueryLogFormatJSON:
		if len(stats.TablesUsed) != 0 {
			fmtString = `{"Method": %q, "RemoteAddr": %q, "Username": %q, "ImmediateCaller": %q, "Effective Caller": %q, "Start": "%v", "End": "%v", "TotalTime": %.6f, "PlanTime": %v, "ExecuteTime": %v, "CommitTime": %v, "StmtType": %q, "SQL": %q, "BindVars": %v, "ShardQueries": %v, "RowsAffected": %v, "Error": %q,  "Keyspace": %q, "Table": %q, "TabletType": %q, "TablesUsed": %v}
`
		} else {
