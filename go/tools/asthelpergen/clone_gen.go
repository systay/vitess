...
func (c *cloneGen) cloneFuncFor(t types.Type) *jen.Statement {
	c.todo = append(c.todo, t)
	return jen.Id("Clone" + printableTypeName(t))
}
...
func (c *cloneGen) makeStructCloneMethod(t types.Type, stroct *types.Struct) error {
	...
	for i := 0; i < stroct.NumFields(); i++ {
		...
		case isSlice(field.Type()) || c.isInterestingType(field.Type()):
			// v: n.Clone()
			values[id] = c.cloneFuncFor(field.Type()).Call(jen.Id("n").Dot(field.Name()))
		case isInterface(field.Type()) || c.isInterestingType(field.Type()):
			// v: CloneAST(n)
			values[id] = c.cloneFuncFor(field.Type()).Call(jen.Id("n"))
		default:
			// v: n.v
...
func (c *cloneGen) makeSliceCloneMethod(t types.Type, slice *types.Slice) error {
	...
	x := jen.Func().Id("Clone"+name).Call(jen.Id("n").Id(typeString)).Id(typeString).Block(
...
func (c *cloneGen) copySliceElement(elType types.Type) jen.Code {
	...
	return jen.For(jen.List(jen.Id("i"), jen.Id("x"))).Op(":=").Range().Id("n").Block(
		jen.Id("res").Index(jen.Id("i")).Op("=").Id("Clone" + printableTypeName(elType)).Call(jen.Id("x")),
	)
}
...
func (c *cloneGen) makeInterface(t types.Type, iface *types.Interface) error {
		...
		case *types.Pointer:
			...
			if !isIface {
				...
				cases = append(cases, jen.Case(jen.Id(typeString)).Block(
					jen.Return(c.cloneFuncFor(t).Call(jen.Id("in"))),
				))
			}
	...
	c.methods = append(c.methods, jen.Func().Id("Clone"+typeName).Call(jen.Id("in").Id(typeString)).Id(typeString).Block(stmts...))
	return nil
}
...
func (c *cloneGen) createFile(pkgName string) (string, *jen.File) {
	...
	for len(c.todo) > 0 {
		...
		typeName := printableTypeName(t)
		c.todo = c.todo[1:]
		_, done := c.addedCloneFor[typeName]
		if done {
			fmt.Println("skipping " + typeName)
			continue
		}
		...
		log.Errorf("don't know how to handle %T", underlying)
	}
