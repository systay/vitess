...
func TestAggregateTypes(t *testing.T) {
	...
	mcmp.Exec("vexplain trace select val1, count(distinct val2), count(*) from aggr_test group by val1")
	mcmp.Exec("select val1, sum(distinct val2), sum(val2) from aggr_test group by val1")
	mcmp.Exec("select val1, count(distinct val2) k, count(*) from aggr_test group by val1 order by k desc, val1")
	mcmp.Exec("select val1, count(distinct val2) k, count(*) from aggr_test group by val1 order by k desc, val1 limit 4")

	mcmp.Exec("select ascii(val1) as a, count(*) from aggr_test group by a")
	mcmp.Exec("select ascii(val1) as a, count(*) from aggr_test group by a order by a")
	mcmp.Exec("select ascii(val1) as a, count(*) from aggr_test group by a order by 2, a")

	mcmp.Exec("select val1 as a, count(*) from aggr_test group by a")
	mcmp.Exec("select val1 as a, count(*) from aggr_test group by a order by a")
	mcmp.Exec("select val1 as a, count(*) from aggr_test group by a order by 2, a")
	mcmp.Exec("select sum(val1) from aggr_test")
