...
func TestMain(m *testing.M) {
	...
	exitCode := func() int {
		...
		keyspace.Shards[0].Vttablets[0].MysqlctlProcess.Stop()
...
func TestScatterErrsAsWarns(t *testing.T) {
	...
	defer func() {
		exec(t, conn, fmt.Sprintf("set workload = oltp"))
		exec(t, conn, "use @master")
		exec(t, conn, `delete from t1`)
	}()
	require.NoError(t, // stop one tablet from the first shard 
		clusterInstance.Keyspaces[0].Shards[0].Vttablets[0].MysqlctlProcess.Stop())
